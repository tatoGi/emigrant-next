...
const SupportChat = () => {
  ...
  const handleSend = () => {
    ...
    // TODO: Replace with real API call to Laravel: POST /api/support/messages
    setTimeout(() => {
      setMessages((prev) => [
        ...prev,
        { text: "გმადლობთ შეტყობინებისთვის! ჩვენი გუნდი მალე დაგიკავშირდებათ.", isUser: false },
      ]);
    }, 1000);
  };
                ...
                {FAQ_ITEMS.map((item, i) => (
                  <div key={i} className="border border-border rounded-xl overflow-hidden">
                    ...
                    >
                      ...
                      <ChevronDown
                        className={`h-4 w-4 text-muted-foreground shrink-0 transition-transform ${activeFaq === i ? "rotate-180" : ""}`}
                      />
                    </button>
                    {activeFaq === i && (
